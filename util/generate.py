...
def generate_fast(
    ...
    o_past_key_values= None
    use_cache=True
    attention_flag = True
    # print(model.config._name_or_path)
    # InternLM   attention_mask=attention_mask[:,cur_context], use_cache=False
    if "Mistral" in model.config._name_or_path:
        attention_flag = False
    if "internlm" in model.config._name_or_path or "InternLM" in model.config._name_or_path or "Mistral" in model.config._name_or_path:
        use_cache=False
    # print(attention_flag, use_cache)
    with torch.no_grad():
        while input_ids.size(1) < max_out_len:  # while not exceeding max output length
            ...
            model_out = model(
                ...
                attention_mask=attention_mask[:,cur_context]  if attention_flag else attention_mask,
            ...
            # print(softmax_out_top_k.sum(1)[:, None])
