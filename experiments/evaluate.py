...
from OrthoEdit import OrthoEditHyperParams
from AlphaEdit.AlphaEdit_main import apply_AlphaEdit_to_model
from OrthoEdit.OrthoEdit_main import apply_OrthoEdit_to_model, get_cov
...
ALG_DICT = {
    "OrthoEdit": (OrthoEditHyperParams, apply_OrthoEdit_to_model),
...
def main(
    ...
    if type(model_name) is str:
        ...
        model = AutoModelForCausalLM.from_pretrained(model_name, use_auth_token=True, device_map='auto', trust_remote_code=True)
        tok = AutoTokenizer.from_pretrained(model_name, use_auth_token=True, trust_remote_code=True)
    ...
    cache_template = None
    if use_cache:
        if any(alg in alg_name for alg in ["OrthoEdit","MEMIT","AlphaEdit", "MEMIT_seq", "MEMIT_prune", "MEMIT_rect"]):
            cache_template = (
                KV_DIR
    ...
    if any(alg in alg_name for alg in ["OrthoEdit","AlphaEdit", "MEMIT_seq", "MEMIT_prune", "NSE"]):
        ...
        if hparams.model_name == "gpt2-xl":
            ...
            if (alg_name == "AlphaEdit") or (alg_name == "OrthoEdit"):
                P = torch.zeros((len(hparams.layers), W_out.shape[0], W_out.shape[0]), device="cpu")
        elif hparams.model_name in ["EleutherAI_gpt-j-6B","Llama3-8B", "Qwen2.5-7B", "phi-3","phi-1.5", "Mistral-7B", "InternLM2-7B", "InternLM-7B"]:
    ...
    Z = []
    if alg_name == "AlphaEdit" or (alg_name == "OrthoEdit"):
        if osp.exists("null_space_project.pt"):
            P = torch.load("null_space_project.pt")
            for i, layer in enumerate(hparams.layers):
                Z.append(P[i,:,:])  # only for test
        else:
            for i, layer in enumerate(hparams.layers):
                P[i,:,:], z = get_project(model_name, model,tok,layer,hparams)
                Z.append(z)
    ...
    if alg_name == "AlphaEdit" or (alg_name == "OrthoEdit"):
        last_k = torch.zeros(([cache_c.shape[1], num_edits]), device="cpu")   # shape [14336, bsz]
    for record_chunks in chunks(ds, num_edits):


        if alg_name == "OrthoEdit":
            #  OrthoEdit Update P and Z
            if cnt > 0:
                for i, layer in enumerate(hparams.layers):
                    P[i, :, :], Z[i] = get_increment_project(last_k[i, :, :], Z[i], hparams)   #  N_t-1
        ...
        etc_args = dict(cache_template=cache_template) if any(alg in alg_name for alg in ["OrthoEdit","ROME", "MEMIT","AlphaEdit", "MEMIT_seq", "MEMIT_prune", "NSE"]) else dict()
        seq_args = dict(cache_c=cache_c) if any(alg in alg_name for alg in ["OrthoEdit", "AlphaEdit", "MEMIT_seq", "NSE"]) else dict()
        nc_args = dict(P = P) if any(alg in alg_name for alg in ["OrthoEdit", "AlphaEdit"]) else dict()
        ...
        start = time()
        if any(alg in alg_name for alg in ["OrthoEdit","AlphaEdit", "MEMIT_seq", "NSE"]):
            edited_model, cache_c, last_k = apply_algo(
                alg_name,
                model,
                tok,
                [
...
def get_project(model_name, model, tok, layer, hparams):
    ...
    print(len(small_singular_indices))   # preserved dim, 0 eigen values
    P = U[:, small_singular_indices] @ U[:, small_singular_indices].T
    Z = U[:, small_singular_indices]

    return P, Z


def get_increment_project(last_k, pre_Z,  hparams):
    print("get increment null space project", last_k.shape, pre_Z.shape)

    B = pre_Z.T @ last_k    #  non-sym   small
    assert not torch.isnan(B).any(), "Matrix contains NaNs"
    assert not torch.isinf(B).any(), "Matrix contains Infs"

    U, S, Vh = torch.linalg.svd(B, full_matrices=True)   #   [null,null]  [bsz, bsz]
    threshold = hparams.nullspace_threshold
    rank = torch.sum(S >= threshold)       # fixed threshold

    if rank >= pre_Z.shape[1]:
        print("*************\n***********\n No Null Space Left \n*************\n*************")
    print("rank: ", rank)   # preserved dim, 0 eigen values
    left_base = U[:, rank:]
    Z = pre_Z @ left_base
    P = Z @ Z.T
    print("new P,Z shape", P.shape, Z.shape)
    return P, Z

def estimate_rank_by_energy(S, energy_threshold=0.9999):

    S_squared = S ** 2
    total_energy = torch.sum(S_squared)
    cumulative_energy = torch.cumsum(S_squared, dim=0)
    energy_ratio = cumulative_energy / total_energy
    q = torch.searchsorted(energy_ratio, energy_threshold).item() + 1
    return q
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--alg_name",
        choices=["OrthoEdit","AlphaEdit","MEMIT_rect", "MEMIT_seq","MEMIT_prune", "MEMIT", "ROME", "FT", "MEND","NSE"],
        default="ROME",
