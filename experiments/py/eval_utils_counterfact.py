...
def test_batch_prediction(
    ...
    with torch.no_grad():
        if router:
            ...
            ori_logits = ori_model(**prompt_tok.to(ori_model.device)).logits
            ori_logits.to(model.device)
            for idx,lab in enumerate(cur_label):
                if lab == 0:
                    logits[idx*2,:,:] = ori_logits[idx*2,:,:]
                    logits[idx*2+1,:,:] = ori_logits[idx*2+1,:,:]
        else:
            logits = model(**prompt_tok).logits
